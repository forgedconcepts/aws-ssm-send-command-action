...
const MockedSendCommand = SendCommandCommand as jest.Mocked<
  typeof SendCommandCommand
>;
  ...
  it('set successful outputs if the status is Success', async () => {
    ...
    const send = jest.fn((arg) => {
      if (arg instanceof SendCommandCommand) {
        ...
        return {
          CommandInvocations: [{Status, CommandPlugins: [{Status, Output}]}],
        };
      }
    });
    ...
    expect(command.DocumentName).toEqual(
      DEFAULT_INPUTS.get('document-name') as string,
    );
    expect(command.Parameters).toEqual(
      JSON.parse(DEFAULT_INPUTS.get('parameters') as string),
    );
